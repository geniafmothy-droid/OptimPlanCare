...
export const ScheduleGrid: React.FC<ScheduleGridProps> = ({ 
  ...
  if (viewMode === 'hourly') {
    ...
    return (
      <div className="flex-1 overflow-hidden flex flex-col h-full relative group bg-white dark:bg-slate-800">
         {holidayName && (
             <div className="bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 text-xs text-center py-1 font-medium border-b border-red-100 dark:border-red-900">
            ...
            <table className="border-collapse w-max min-w-full">
              <thead className="sticky top-0 z-20 bg-white dark:bg-slate-800 shadow-sm">
                 <tr>
                    ...
                    {hours.map(h => (
                       <th key={h} className="min-w-[40px] border-b border-r border-slate-200 dark:border-slate-700 p-1 text-center text-xs bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300">
                          {h}h
                       </th>
                    ))}
                 </tr>
          ...
          <tbody>
            {employees.map((emp) => (
              <tr key={emp.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                <td className="sticky left-0 z-10 bg-white dark:bg-slate-800 border-b border-r border-slate-200 dark:border-slate-700 p-2 group cursor-pointer h-12">
