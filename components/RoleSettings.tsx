...
import { Users, Edit2, Shield, Briefcase, CheckCircle2, ChevronDown, ChevronUp, Plus, X, Stethoscope, FileText, Baby } from 'lucide-react';
...
export const RoleSettings: React.FC = () => {
    ...
    const loadRoles = async () => {
        const data = await db.fetchRoles();
        setRoles(data);
    };
    ...
    const handleSave = async () => {
        ...
        try {
            await db.upsertRole(editingRole);
            loadRoles();
            setEditingRole(null);
    ...
    const getRoleIcon = (code: string) => {
        switch(code) {
            ...
            case 'SAGE_FEMME': return <Baby className="w-5 h-5"/>;
            case 'SECRETAIRE': 
    ...
    const getRoleColor = (code: string) => {
        switch(code) {
            ...
            case 'SAGE_FEMME': return 'bg-rose-100 text-rose-600';
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {roles.map(role => (
                        <div key={role.id} className="border rounded-lg p-4 flex items-center justify-between hover:shadow-md transition-all">
                            ...
                            <button onClick={() => setEditingRole(role)} className="px-3 py-1.5 border rounded-lg hover:bg-slate-50 text-slate-600 flex items-center gap-1 text-xs">
                    ...
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input type="text" value={editingRole.label} onChange={e => setEditingRole({...editingRole, label: e.target.value})} className="w-full p-2 border rounded"/>
                            </div>
                            <div>
                                <label className="block text-sm font-medium mb-1">Description</label>
                                <textarea value={editingRole.description} onChange={e => setEditingRole({...editingRole, description: e.target.value})} className="w-full p-2 border rounded" rows={3}/>
                            </div>
                        </div>
